...
uint8_t binArr[] = { 0xAA, // 10101010 in binary
...
int main(void) {
      ...
      for (uint16_t address = 0; address < sizeof(binArr); address++) {
            write_to_address(address, binArr[address]);
      }
...
// Timer rolled over
      void TIM16_IRQHandler(void) {
            ...
            uint8_t data = read_from_address(Index);
            set_leds(data);
            if (data != binArr[Index]) {
            ...
            Index++;
            if (Index >= sizeof(binArr)) {
                  Index = 0; // back to the beginning
            }
