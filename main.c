...
int main(void)
{
 ...
 write_to_address(addresses, patterns[i]);
 addresses++;
 }
 /* USER CODE END 2 */
 /* Infinite loop */
...
if(htim16.Init.Period == 1000-1 && Delay==0)
 {
...
else if(htim16.Init.Period == 500 && Delay==0)
 {
...
static void init_spi(void) {
 ...
 // Set pin modes
 GPIOB->MODER |= GPIO_MODER_MODER13_1; // Set pin SCK (PB13) to Alternate
Function
 GPIOB->MODER |= GPIO_MODER_MODER14_1; // Set pin MISO (PB14) to Alternate
Function
 GPIOB->MODER |= GPIO_MODER_MODER15_1; // Set pin MOSI (PB15) to Alternate
Function
...
Set Baud to fpclk / 16
 SPI2->CR1 |= SPI_CR1_MSTR;
// Set to master mode
 SPI2->CR2 |= SPI_CR2_FRXTH;
// Set RX threshold to be 8 bits
 SPI2->CR2 |= SPI_CR2_SSOE;
// Enable slave output to work in master mode
...
dummy = SPI2->DR;
// Clock in the data
*((uint8_t*)(&SPI2->DR)) = 0x42; // Clock out some
junk data
while ((SPI2->SR & SPI_SR_RXNE) == 0); // Hang while RX is empty
dummy = SPI2->DR;
GPIOB->BSRR |= GPIO_BSRR_BS_12; // Pull CS high
...
// TODO: Change to next LED pattern; output 0x01 if the read SPI data is
incorrect
address_data = read_from_address(index);
if (address_data == patterns[index]){
GPIOB -> ODR = patterns[index];
}
else {
GPIOB -> ODR = 0b11111101;
}
index++;
if (index >= pattsize){
index = 0;
